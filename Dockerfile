...
COPY ["VotingPoll.API.csproj", "VotingPoll.API/"]
COPY ["VotingPoll.Core.csproj", "VotingPoll.Core/"]
COPY ["VotingPoll.Infrastructure.csproj", "VotingPoll.Infrastructure/"]
RUN dotnet restore "VotingPoll.API/VotingPoll.API.csproj"

# Now copy everything else and publish a Release build.
COPY . .
RUN dotnet publish "VotingPoll.API/VotingPoll.API.csproj" -c Release -o /app/publish
