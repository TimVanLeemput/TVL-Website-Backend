...
public class VotingServiceTests
{
    ...
    [Fact]
    public async Task CreateVote_PollClosed_VotingStillAllowed()
    {
        ...
        Poll poll = new Poll
        {
            Id = 1,
            Title = "Test Poll",
            AllPollOptions = new List<PollOption> { pollOption },
            ClosesAt = DateTime.UtcNow.AddDays(-1)
        };

        _pollRepoMock.Setup(r => r.GetByIdAsync(1)).ReturnsAsync(poll);
        _pollOptionRepoMock.Setup(r => r.GetAsync(1, 1)).ReturnsAsync(pollOption);
        _voteRepoMock.Setup(r => r.UserAlreadyVotedAsync(1, 1)).ReturnsAsync(false);
        _voteRepoMock.Setup(r => r.CreateAsync(It.IsAny<Vote>())).ReturnsAsync((Vote v) => v);
